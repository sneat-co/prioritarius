...
@Component({
  selector: 'prioritarius-lists-page',
...
export class ListsPageComponent extends SpaceBaseComponent {
  private readonly params = inject(PrioritariusComponentBaseParams);
  ...
  constructor() {
    ...
    this.prioritariusAppStateService.changed.subscribe((appState) => {
  ...
  public clickGroup(group: IListGroup): void {
    ...
    this.prioritariusAppStateService.setGroupCollapsed(
      group.title,
    ...
    try {
      ...
      if (this.space) {
        if (this.reordered) {
          ...
          const prioritariusDbo = this.space.dbo as unknown as
            | IPrioritariusSpaceDbo
            | undefined;
          this.updateListsFromSpace(prioritariusDbo?.listGroups); // persisted lists
        }
      } else {
        this.listGroups = [];
      }
