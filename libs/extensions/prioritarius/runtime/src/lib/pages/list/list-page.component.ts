...
import { PrioritariusCoreServicesModule } from '../../services';
...
import { PrioritariusComponentBaseParams } from '../../prioritarius-component-base-params';
import { IPrioritariusAppStateService } from '../../services';
...
@Component({
  selector: 'prioritarius-list',
  templateUrl: './list-page.component.html',
  imports: [
    PrioritariusCoreServicesModule,
    NgOptimizedImage,
    ListItemComponent,
  ...
  providers: [
    ...
    PrioritariusComponentBaseParams,
    ListDialogsService,
    RandomIdService,
  ],
...
export class ListPageComponent extends BaseListPage implements AfterViewInit {
  ...
  private readonly prioritariusAppStateService = inject(IPrioritariusAppStateService);
  ...
  constructor() {
    const params = inject(PrioritariusComponentBaseParams);

    super(params);
    const prioritariusAppStateService = this.prioritariusAppStateService;
    this.preloader.markAsPreloaded('list');
    if (location.pathname.includes('/lists')) {
      // TODO: document why & how it is possible
      return;
    }
    prioritariusAppStateService.changed.subscribe((appState) => {
  ...
  public clickShowWatchedMovies(): void {
    this.prioritariusAppStateService.setShowWatched(this.isHideWatched());
  }
  ...
  // 	if (this.appService.appId === 'prioritarius') {
