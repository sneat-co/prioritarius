...
export interface IPrioritariusAppState {
  collapsedGroups: string[];
  showWatched: boolean;
}

export abstract class IPrioritariusAppStateService extends LocalAppState<IPrioritariusAppState> {
...
@Injectable()
export class PrioritariusAppStateService extends IPrioritariusAppStateService {
  constructor() {
  ...
  public setShowWatched(showWatched: boolean): void {
    const appState: IPrioritariusAppState = {
      ...this.getAppState(),
      showWatched,
    };
    this.saveAppState(appState);
