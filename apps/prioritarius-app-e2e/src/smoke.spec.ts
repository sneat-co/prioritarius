...
// crashing. Full "renders goals data" needs an authenticated session + seeded
...
test('space-scoped goals route loads without crashing', async ({ page }) => {
  const pageErrors: string[] = [];
  page.on('pageerror', (e) => pageErrors.push(String(e)));

  // Lazy-loads the space shell + goals route. Unauthenticated, this redirects
  // to login; the assertion is that the app handles it without throwing.
  await page.goto('/space/family/smoke-test-space/goals');
